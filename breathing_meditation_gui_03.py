...
MIN_LIGHT_RADIUS = 5.0      # Minimum radius when exhaled
MAX_LIGHT_RADIUS = 20.0     # Maximum radius when inhaled
...
class BreathingMeditationGUI:
    ...
    def get_player_color(self, phase, phase_progress):
        """Get player color based on breathing phase with brightness variation."""
        if phase == "INHALE":
            # Cyan - starts dim, gets brighter
            brightness = int(100 + 155 * phase_progress)
            return f'#{brightness:02x}{brightness:02x}FF'
        elif phase == "HOLD":
            # Yellow at full brightness
            return '#FFFF00'
        else:  # EXHALE
            # Blue - starts bright, gets dimmer (navy)
            brightness = int(100 + 155 * (1.0 - phase_progress))
    ...
    def render_maze(self):
        ...
        phase, phase_time, phase_duration, phase_progress = self.breathing_state.get_breathing_phase()
        ...
        for r in range(self.maze.height):
            for c in range(self.maze.width):
                ...
                if r == self.player.row and c == self.player.col:
                    ...
                    color = self.get_player_color(phase, phase_progress)
